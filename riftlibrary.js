      ...
      <div class="edit-grid">
        <div>
          <div class="edit-bar">
            ...
            <span id="edit-total-count" style="color:var(--accent);">${totalCards} cards</span>
          </div>
          <div class="edit-search-wrap">
            <span class="edit-si">⌕</span>
            <input type="text" id="edit-search" placeholder="Card name…" oninput="renderEditSearch()" style="width:100%;padding-left:34px;">
          </div>
          <div class="edit-card-list" id="edit-card-list"></div>
        </div>
        <div>
          <div class="edit-preview">
            <div class="edit-preview-title">Current deck <span style="color:var(--text-muted);font-weight:400;">(${totalCards}/40)</span></div>
            <div class="edit-preview-list" id="edit-preview-list"></div>
          </div>
        </div>
      </div>
    </div>
...
function renderEditSearch(){
  const d=myDecks.find(x=>x.id===activeDeckId);if(!d)return;
  const q=(document.getElementById('edit-search')?document.getElementById('edit-search').value:'').toLowerCase().trim();
  const list=document.getElementById('edit-card-list');if(!list)return;
  if(!cardsLoaded||!CARDS.length){
    list.innerHTML='<div style="text-align:center;padding:1.5rem;color:var(--text-muted);">'
      +'<div style="font-size:13px;margin-bottom:8px;">Cards are still loading from Riftcodex…</div>'
      +'<button class="btn btn-sm btn-g" onclick="fetchAllCards()">Load now</button>'
      +'</div>';
    return;
  }
  const source=q?CARDS.filter(c=>c.name.toLowerCase().includes(q)).slice(0,40):CARDS.slice(0,40);
  if(!source.length){list.innerHTML='<p style="font-size:13px;color:var(--text-muted);padding:8px;">No cards match that search.</p>';return;}
  var html='';
  source.forEach(function(c){
    ...
    html+='<div class="edit-card-row">'
      +'<span class="pill '+c.dom+'" style="font-size:10px;flex-shrink:0;">'+domCap+'</span>'
      +'<span class="edit-card-name">'+c.name+'</span>'
      +'<span class="edit-card-type">'+c.type+'</span>'
      +'<div style="display:flex;align-items:center;gap:4px;">'
      +'<button class="edit-qty-btn" onclick="editDeckCard(\''+c.id+'\',\''+safeName+'\',\''+c.type+'\',-1)">−</button>'
      +'<span class="edit-qty">'+(cnt||0)+'</span>'
      +'<button class="edit-qty-btn" onclick="editDeckCard(\''+c.id+'\',\''+safeName+'\',\''+c.type+'\',1)">+</button>'
      +'</div></div>';
  });
  list.innerHTML=html;
...
function renderEditPreview(){
  const d=myDecks.find(x=>x.id===activeDeckId);if(!d)return;
  const prev=document.getElementById('edit-preview-list');if(!prev)return;
  const cards=d.cards||[];
  const total=cards.reduce((a,c)=>a+c.cnt,0);
  const countEl=document.getElementById('edit-total-count');
  if(countEl) countEl.textContent=total+' cards';
  if(!cards.length){prev.innerHTML='<p style="font-size:12px;color:var(--text-muted);">Empty deck</p>';return;}
  const sorted=cards.slice().sort(function(a,b){return a.t.localeCompare(b.t)||a.n.localeCompare(b.n);});
  var html='';
  sorted.forEach(function(c){
    html+='<div class="epl-row"><span class="epl-name">'+c.n+'</span><span class="epl-cnt">×'+c.cnt+'</span></div>';
  });
  prev.innerHTML=html;
}
...
function editDeckCard(cardId,cardName,cardType,delta){
  ...
  if(idx>=0){
    ...
    const total=d.cards.reduce((a,c)=>a+c.cnt,0);
    if(total>=60){toast('Deck is very large already');return;}
